...
def get_ref_point_deterministe(point, ListFace, Face, c):
...
def Q_Learning_train_random(ScoreMin, NBTest, shape, gamma):#RatioMin, NBegal, gain, PoidInitial):
...
def Q_Learning_execute2(Data, shape):
    gmsh.clear()
    file_name = shape + ".txt"
    f = open(file_name, "r")
    ps = create_shape(f)
...
def Search_Param(shape):
    ...
    ScoreMin = 9999
    continuer = True
    RatioTest = [1, 7, 15, 22, 30]

    while continuer:
        score1 = 0
        score2 = 0
        Data = Q_Learning_train_Algo(RatioTest[1]/10, NBTest, shape, 0.1)
        for j in range(31, 41):
            score1 += Q_Learning_execute2(Data, "shape_" + str(j))

        print("Score moyen pour NBTest = ", RatioTest[1], ": ", score1 / 10)

        Data = Q_Learning_train_Algo(RatioTest[1]/10, NBTest, shape, 0.1)
        for j in range(31, 41):
            score2 += Q_Learning_execute2(Data, "shape_" + str(j))

        print("Score moyen pour NBTest = ", RatioTest[1], ": ", score2 / 10)

        if score1 >= score2:
            if RatioTest[0] == RatioTest[1]:
                continuer = False
                score2 = 0
                Data = Q_Learning_train_Algo(RatioTest[1]/10, NBTest, shape, 0.1)
                for j in range(31, 41):
                ...
                print("Score moyen pour NBTest = ", RatioTest[1], ": ", score1 / 10)
                if score1 >= score2:
                    RatioMin = RatioTest[1]/10
                else:
                    RatioMin = RatioTest[2]/10

            RatioTest[1] = round(RatioTest[1] / 2)
            RatioTest[2] = RatioTest[1]
            RatioTest[3] = RatioTest[1] * 3
            RatioTest[4] = RatioTest[3]
        else:
            if RatioTest[0] == RatioTest[1]:
                continuer = False
                score1 = 0
                ...
                if score1 >= score2:
                    ...
                    RatioMin = RatioTest[3]/10

            RatioTest[1] = round((RatioTest[4] - RatioTest[0]) / 4 + RatioTest[0])
            RatioTest[2] = RatioTest[3]
            RatioTest[3] = round(((RatioTest[4] - RatioTest[0]) * 3) / 4 + RatioTest[0])
            RatioTest[0] = RatioTest[2]
    ...
    """ScoreMin = 9999
    continuer = True
    RatioTest = [1, 25, 50, 75, 100]

    while continuer:
        score1 = 0
        score2 = 0
        Data = Q_Learning_train_random(1.2, RatioTest[1], shape, 0.1)
        for j in range(31, 41):
            score1 += Q_Learning_execute(Data, "shape_" + str(j))

        print("Score moyen pour NBTest = ", RatioTest[1], ": ", score1 / 10)

        Data = Q_Learning_train_random(1.2, RatioTest[3], shape, 0.1)
        for j in range(31, 41):
            score2 += Q_Learning_execute(Data, "shape_" + str(j))
        ...
        if score1 >= score2:
            if RatioTest[0] == RatioTest[1]:
                continuer = False
                score2 = 0
                Data = Q_Learning_train_random(1.2, RatioTest[2], shape, 0.1)
                for j in range(31, 41):
                    score2 += Q_Learning_execute(Data, "shape_" + str(j))
            ...
            RatioTest[1] = RatioTest[round(RatioTest[1]/2)]
            RatioTest[2] = RatioTest[1]
            RatioTest[3] = RatioTest[RatioTest[1] * 3]
            RatioTest[4] = RatioTest[3]
        else:
            if RatioTest[0] == RatioTest[1]:
                continuer = False
            ...
            RatioTest[1] = RatioTest[round((RatioTest[4]-RatioTest[0])/4 + RatioTest[0])]
            RatioTest[2] = RatioTest[3]
            RatioTest[3] = RatioTest[round(((RatioTest[4]-RatioTest[0])*3)/4 + RatioTest[0])]
            RatioTest[0] = RatioTest[2]
    ...
    """for i in range(10, 100, 10):
        Data = Q_Learning_train_random(1.2, i, shape, 0.1)
    ...
    """for i in range(10,50):
        Data = Q_Learning_train_random(i/10, 10, shape,0.1)
        ...
        for j in range(30, 41):
            print("exectute shape: ", j)

            add =  Q_Learning_execute(Data, "shape_" + str(j))
    ...
    """# cherche le Gamma idéal
    ScoreMin = 9999
    gamma = 0
    for i in range(1,10):
        Data = Q_Learning_train_random(RatioMin, 10, shape,i/10)
        ...
        print("Score moyen pour Gamma = ", i/10, ": ", score / 10)
        if score < ScoreMin:
            ScoreMin = score
            gamma = i/10"""
        ...
        for nb in range(NBTest):
            ...
            while faces:
                ...
                for p in points_candidats:
                    ref = get_ref_point_deterministe(p, faces, f, centre)
                ...
                ref = get_ref_point_deterministe(p, get_all_non_square_faces(), f, centre)
...
def Q_Learning_execute2(Data, shape):
    ...
    while len(Face) != 0:  # and nbbcl != 10:
        ...
        if len(PointValide) == 1:
            p = PointValide[0]
            ref = get_ref_point_deterministe(p, Face, f_tag, c)
            print("1 point valide")
        else:
            #selectionne le point dont la somme des poids est la plus élevé
            MaxSomme = [-1, 0, ""]
...
if __name__ == '__main__':
    ...
    #Data = Q_Learning_train_random(3.2, 10, shape,0.1)
    Data = Q_Learning_train_Algo(1, 3, shape, 0.1)
    ...
    for i in range(41, 42):
        ...
        score += Q_Learning_execute2(Data, "shape_" + str(i))
    ...
    print ("l'entrainement a duré ", end - start)
